...
const renderList = list => {
  ...
  list.forEach(({ item, isDone, id }) => {
    ...
    itemElem.innerHTML = `${item} <button data-id=${id} class="delete badge badge-danger badge-pill">Devour</button>`
...
const updateItem = ({ id, dataset: { isDone } }) => {
  ...
  axios.put(`/api/items/${id}`, { isDone: !parseInt(isDone) })
    ...
    .then(({ data: user }) => renderList(user.items))
    // handle our errors
...
const createUser = () => {
  ...
  axios.post('/api/users', {
    ...
    .then(({ data: user }) => {
      ...
      document.getElementById('message').textContent = `Hello ${user.username}! Welcome!`
